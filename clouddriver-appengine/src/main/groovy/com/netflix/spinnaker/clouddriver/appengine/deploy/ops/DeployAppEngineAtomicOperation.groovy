...
class DeployAppEngineAtomicOperation implements AtomicOperation<DeploymentResult> {
  ...
  String cloneOrUpdateLocalRepository(String directoryName, Integer retryCount) {
    ...
    try {
      ...
      runCommand(["git", "-C", directoryName, "checkout", branch])
      runCommand(["git", "-C", directoryName, "merge", "origin/$branch"])
    } catch (Exception e) {
      directory.deleteDir()
      if (retryCount > 0) {
  ...
  String deploy(String directoryName) {
    ...
    def serverGroupNameResolver = new AppEngineServerGroupNameResolver(project, description.credentials)
