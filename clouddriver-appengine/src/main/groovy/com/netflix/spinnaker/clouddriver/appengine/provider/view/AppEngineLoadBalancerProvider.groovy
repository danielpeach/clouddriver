...
class AppEngineLoadBalancerProvider implements LoadBalancerProvider<AppEngineLoadBalancer> {
  ...
  Set<AppEngineLoadBalancer> translateLoadBalancers(Collection<CacheData> cacheData) {
    cacheData.collect { loadBalancerData ->
      Set<AppEngineInstance> instances = AppEngineProviderUtils.resolveRelationshipData(cacheView, loadBalancerData, Namespace.INSTANCES.ns)
        .collect { AppEngineProviderUtils.instanceFromCacheData(objectMapper, it) }

      Set<AppEngineServerGroup> serverGroups = AppEngineProviderUtils.resolveRelationshipData(cacheView, loadBalancerData, Namespace.SERVER_GROUPS.ns)
        .collect { AppEngineProviderUtils.serverGroupFromCacheData(objectMapper, it, instances)}
